        ...
        for page_num in range(len(doc)):
            ...
            for inst in text_instances:
                ...
                if match:
                    ...
                    placeholders.append(
                        (f"P#{placeholder_id}", inst.x0, inst.y0, page_num)
                    )
            ...
            for i, line in enumerate(lines):
                if line.startswith("P#"):
                    ...
                    if i + 1 < len(lines):
                        ...
                        if not re.match(
                            r"^\d+(\.\d+)*$", next_line
                        ) and not next_line.startswith("P#"):
                    ...
                    for head_text, page_no in heading_pages.items():
                        ...
                        heading_clean = (
                            full_heading.lower().replace(" ", "").replace(".", "")
                        )
                        head_clean = head_text.lower().replace(" ", "").replace(".", "")

                        # Try different matching strategies
                        if (
                            heading_clean in head_clean
                            or head_clean in heading_clean
                            or full_heading.lower() in head_text.lower()
                            or head_text.lower() in full_heading.lower()
                            or heading_text.lower() in head_text.lower()
                        ):
                            actual_page = page_no
                            break
                    ...
                    if actual_page and actual_page > 1:  # Skip title page
                        ...
                        placeholder_num = int(
                            re.search(r"P#(\d+)", placeholder).group(1)
                        )
                        estimated_page = max(
                            3, (placeholder_num // 5) + 2
                        )  # Rough estimate
                        toc_structure.append(
                            {
                                "heading": full_heading,
